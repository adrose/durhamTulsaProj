...
## mod.five.sub
mod.five.pk.1 <- lmerTest::lmer(value ~ (questionTarg + questionVal + condition)^3 + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="Yes"),], REML=TRUE, na.action = "na.exclude") 
mod.five.pk.2 <- lmerTest::lmer(value ~ (questionTarg + questionVal + condition)^3 + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="No"),], REML=TRUE, na.action = "na.exclude") 


## Now look within pk == yes
mod.five.pk.1.black <- lmerTest::lmer(value ~ condition  + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="Yes" & data.hlm$questionTarg=="Black Tulsans"),], REML=TRUE, na.action = "na.exclude") 
mod.five.pk.1.police <- lmerTest::lmer(value ~ condition  + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="Yes" & data.hlm$questionTarg=="Police"),], REML=TRUE, na.action = "na.exclude") 
mod.five.pk.1.white <- lmerTest::lmer(value ~ condition  + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="Yes" & data.hlm$questionTarg=="White Tulsans"),], REML=TRUE, na.action = "na.exclude") 
...
data.hlm$Valence <- data.hlm$condition
p1 <- summarySE(data = data.hlm, measurevar = "predVal", groupvars = c("priorknowledge", "questionTarg", "Valence"), na.rm=T) %>% 
  ggplot(., aes(x=priorknowledge, y=predVal, group=Valence, fill=Valence)) +
  ...
  scale_fill_grey()
ggsave(filename = "./figures/fig1.png", dpi=600, height = 5, width=7)
