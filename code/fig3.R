...
mod.five <- lmerTest::lmer(value ~ (priorknowledge + questionTarg + questionVal + condition)^4 + cohort + (1|id), data=data.hlm, REML=TRUE, na.action = "na.exclude") ## sig interaction here
## mod.five.sub
mod.five.pk.1 <- lmerTest::lmer(value ~ (questionTarg + questionVal + condition)^4 + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="Yes"),], REML=TRUE, na.action = "na.exclude") 
mod.five.pk.2 <- lmerTest::lmer(value ~ (questionTarg + questionVal + condition)^4 + cohort + (1|id), data=data.hlm[which(data.hlm$priorknowledge=="No"),], REML=TRUE, na.action = "na.exclude") 

## Now compare prior knowldeg within each target group
mod.five.black <- lmerTest::lmer(value ~ priorknowledge + questionVal + condition + cohort + (1|id), data=data.hlm[which(data.hlm$questionTarg=="Black Tulsans"),], REML=TRUE, na.action = "na.exclude") ## sig interaction here
mod.five.white <- lmerTest::lmer(value ~ priorknowledge + questionVal + condition + cohort + (1|id), data=data.hlm[which(data.hlm$questionTarg=="White Tulsans"),], REML=TRUE, na.action = "na.exclude") ## sig interaction here
mod.five.police <- lmerTest::lmer(value ~ priorknowledge + questionVal + condition + cohort + (1|id), data=data.hlm[which(data.hlm$questionTarg=="Police"),], REML=TRUE, na.action = "na.exclude") ## sig interaction here
...
ggsave(plot = p3, filename = "./figures/fig3.png", dpi=600)
